document.addEventListener('DOMContentLoaded', function(){
  ...
  const buildPreviewImage = (dataIndex, blob) =>{
    ...
    deleteButton.innerText = "画像の取り消し";
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'spot[images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="spot[images][]"]:last-child');
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="spot[images][]"]');
